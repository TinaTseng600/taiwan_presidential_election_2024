...
connection.close()
total_votes = votes_by_village["sum_votes"].sum() #計算以village為單位的得票率
country_percentage = votes_by_village.groupby("number")["sum_votes"].sum() / total_votes #計算每個候選人1, 2, 3的得票率
vector_a = country_percentage.values #計算全國得票率，以一列作為表示

groupby_variables = ["county", "town", "village"]
village_total_votes = votes_by_village.groupby(groupby_variables)["sum_votes"].sum().reset_index() #一個村鄰里一個加總票數
merged = pd.merge(votes_by_village, village_total_votes, left_on=groupby_variables, right_on=groupby_variables,
                  how="left") #計算每個村鄰里，三個候選人的得票數
merged["village_percentage"] = merged["sum_votes_x"] / merged["sum_votes_y"] #計算村鄰里得票率
